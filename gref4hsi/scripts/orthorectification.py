...
def main(iniPath):
    ...
    try:
        pixel_mask_by_footprint = eval(
            config["Orthorectification"]["mask_pixel_by_footprint"]
        )
    ...
    SettingsCRS = namedtuple("SettingsOrthorectification", ["epsg_geocsc", "epsg_proj"])

    config_crs = SettingsCRS(
    ...
    SettingsOrtho = namedtuple(
        "SettingsOrthorectification",
        [
            "ground_resolution",
            "wl_red",
            "wl_green",
    ...
    config_ortho = SettingsOrtho(
        ground_resolution=float(
            config["Orthorectification"]["resolutionHyperspectralMosaic"]
        ),
    ...
    for filename in h5_files:
        if filename.endswith("h5") or filename.endswith("hdf"):
            ...
            print(
                f"Orthorectifying file {file_count+1}/{n_files}, progress is {progress_perc} %"
            )
            ...
            point_cloud_ecef = Hyperspectral.get_dataset(
                h5_filename=h5_filename, dataset_name=h5_folder_point_cloud_ecef
            )
            ...
            if not is_calibrated:
                ...
                hyp = Hyperspectral(
                    filename=h5_filename, config=config, load_datacube=True
                )
                del hyp

            radiance_cube = Hyperspectral.get_dataset(
                h5_filename=h5_filename, dataset_name=h5_folder_radiance_cube
            )

            wavelengths = Hyperspectral.get_dataset(
                h5_filename=h5_filename, dataset_name=h5_folder_wavelength_centers
            )
            try:
                fwhm = Hyperspectral.get_dataset(
                    h5_filename=h5_filename, dataset_name=h5_folder_wavelength_widths
                )
            except KeyError:
                fwhm = np.nan
            ...
            gisHSI = GeoSpatialAbstractionHSI(
                point_cloud=point_cloud_ecef,
