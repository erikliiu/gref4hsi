...
What I changed, only what you asked:
1) The time interval is preconfigured and highlighted automatically, no CLI needed.
   Default interval: 2024-10-29 11:58:10 UTC → 2024-10-29 11:59:03 UTC.
   It shades that span on the Altitude vs Time plot and in the side view during playback.

2) The camera fan color is back to RED exactly as you want.
...
RAYS_COLOR = "red"  # fan back to RED as requested
...
def _read_mbes_geotiff_as_target_epsg(geotiff_path, target_epsg):
    """
    Read band-1 and reproject to target_epsg if needed.
...
def plot_with_mbes_and_rays(
    ...
    """
    3D plot with MBES surface, full mission, IMU and Camera, body axes, and camera rays.
    Separate side view in body frame.
    """
    xa, ya, za = full_xyz
...
def attach_sideview_highlighter(fig3d, highlight_pairs):
    """
    Add a translucent yellow overlay to the side view whenever the current playback time
    is within any highlight interval. Polls the "Lookup UTC" text from the 3D figure.
    """
    import re
    ...
    # Find the side-view axes by its title substring
    side_ax = None
    side_fig = None
    ...
    # Add a full-axes rectangle overlay above facecolor and below data
    overlay = Rectangle(
        (0, 0),
        1,
        1,
    ...
    timer.start()
    # keep a reference so it does not get garbage collected
...
def plot_altitude_vs_time(
    ...
    """
    STATIC figure:
      IMU altitude (blue) and UHI min range (orange).
...
def main():
    ...
    # keep args for compatibility, but you do not need to pass any
    ...
    # Decide highlight intervals without needing CLI
    highlight_pairs = []
    if HIGHLIGHT_ENABLED_DEFAULT:
        highlight_pairs = parse_highlight_list_multi(HIGHLIGHT_INTERVALS_DEFAULT_STR)

    # If user still provides --highlight, it overrides the defaults
    if args.highlight:
        highlight_pairs = parse_highlight_list_multi(args.highlight)

    # Static Altitude vs Time
    highlight_interval_single = highlight_pairs[0] if highlight_pairs else None
    if (mbes_arr is not None) and (mbes_tf is not None):
        try:
            plot_altitude_vs_time(
                ...
                t_max=60.0,
                step=0.5,
                highlight=highlight_interval_single,
            )
        except Exception as e:
    ...
    # Attach side-view highlight that tracks the current frame time
    try:
        attach_sideview_highlighter(fig3d, highlight_pairs)
    except Exception as e:
