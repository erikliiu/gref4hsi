...
sys.path.append(
    str(Path(__file__).parent.parent / "other" / "legacy_files" / "eely_log_plot")
)
...
def plot_attitude_and_dvl_stacked(nav_df, db_path, t_start, t_end):
    ...
    sig_x = db.get_signal("/sensors/dvl/velocity/D__x", start_time=t_start, end_time=t_end, reset_time_stamp_to_zero=False)
    sig_y = db.get_signal("/sensors/dvl/velocity/D__y", start_time=t_start, end_time=t_end, reset_time_stamp_to_zero=False)
    sig_z = db.get_signal("/sensors/dvl/velocity/D__z", start_time=t_start, end_time=t_end, reset_time_stamp_to_zero=False)
    ...
    ax_att_left.plot(utc_times, nav_df["roll"], "g-", linewidth=1, label="Roll", alpha=0.9)
    ax_att_left.plot(utc_times, nav_df["pitch"], "b-", linewidth=1, label="Pitch", alpha=0.9)
    ...
    # Right Y-axis: Yaw
    ax_att_right.plot(utc_times, nav_df["yaw"], "r-", linewidth=1, label="Yaw", alpha=0.9)
    ...
    # OPTION 1: Above the plot (Best for papers) - CURRENTLY ACTIVE
    ax_dvl.legend(loc="upper center", bbox_to_anchor=(0.5, 1.25), 
                  ncol=3, frameon=False, fontsize=9)
    ...
    print(f"      D__x: μ={np.mean(sig_x.data):.3f} m/s, σ={np.std(sig_x.data):.3f} m/s")
    print(f"      D__y: μ={np.mean(sig_y.data):.3f} m/s, σ={np.std(sig_y.data):.3f} m/s")
    print(f"      D__z: μ={np.mean(sig_z.data):.3f} m/s, σ={np.std(sig_z.data):.3f} m/s")
