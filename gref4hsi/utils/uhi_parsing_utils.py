...
class TimeData:
    ...
    def interpolate(self, time_interp):
        ...
        self.value_interp = interp1d(
            x=self.time, y=self.value, kind="nearest", fill_value="extrapolate"
        )(x=self.time_interp)
...
def unix_to_date_and_time(unix_timestamps):
    """ """
    formatted_dates = [
        datetime.utcfromtimestamp(ts).strftime("%d.%m.%Y") for ts in unix_timestamps
    ]
    formatted_times = [
        datetime.utcfromtimestamp(ts).strftime("%H:%M:%S.%f")[:-3]
...
def write_immersion_compatible_format_from_nav(
    nav, unix_times_rov, time_offset, nav_filename
):
    """
...
def write_data_to_h5_file(h5_filename, h5_dict_write, h5_dict_data):
    with h5py.File(h5_filename, "a", libver="latest") as f:
        ...
        for key, h5_folder in h5_dict_write.items():
            ...
            dset = f.create_dataset(name=h5_folder, data=h5_dict_data[key])


def immersion_filename_to_unix_time(immersion_file_name):

    list_names = immersion_file_name.split(
        "_"
    )  # The year format should start with 202* or 201*

    result = [
        (entry, idx)
        for (idx, entry) in enumerate(list_names)
        if entry.startswith(("202", "201"))
    ]
...
def set_camera_model(
    config, config_file_path, config_uhi, model_type, binning_spatial, fov_arr=None
):
    """
    Creates an XML calibration file that describes the hyperspectral camera's geometry and position on the robot.
    """
    ...
    if model_type == "cal_txt":
        ...
        param_dict = read_fov_file_to_param(
            LAB_CAL_DIR=LAB_CAL_DIR, binning_spatial=binning_spatial
        )
    ...
    # note that config_uhi is not data from .ini but from a namedtuple
    ...
    # takes the camera parameters and writes them to an XML file
    CalibHSI(file_name_cal_xml=xml_cal_write_path, mode="w", param_dict=param_dict)
    """
    Creates and writes an XML calibration file for the hyperspectral camera.

    What it does:
    Takes all the camera parameters that were assembled in param_dict, which includes:

    Field of view angles for each pixel column
    Camera rotation (rx, ry, rz) relative to robot body
    Camera position/translation (tx, ty, tz) on the robot
    Writes them to an XML file at the path specified by xml_cal_write_path

    Example path: /Input/Calib/HSI_2b.xml (for 2x binning)
    The XML file format is readable by the georeferencing software later in the pipeline

    Why XML format?
    XML is a structured format that can store all the camera calibration parameters in an organized way. The georeferencing code later reads this XML file to understand:

    How each pixel maps to a viewing angle
    How the camera is oriented on the robot
    Where the camera is positioned on the robot
    What happens next:
    After creating the XML file, the code updates the configuration to point to this new calibration file:
    config.set("Relative Paths", "hsi_calib_path", value="Input/Calib/" + file_name_xml)    
    So CalibHSI is essentially a writer function that converts the Python dictionary of camera parameters into a standardized XML calibration file that the rest of the pipeline can use for precise georeferencing.
    """
...
def read_nav_from_mat(mat_filename):
    ...
    nav.roll = TimeData(
        time=mat_contents["TELEMETRY"]["SpotOnTime"],
    ...
    nav.yaw = TimeData(
        time=mat_contents["POSITION_RENAV"]["SpotOnTime"],
        value=mat_contents["POSITION_RENAV"]["USBLCourse"],
    )  # The USBL course
    ...
    nav.altitude = TimeData(
        time=mat_contents["ALTIMETER"]["SpotOnTime"],
...
def read_nav_from_dvl_imu_alti(
    dvl_filename, imu_filename, alti_filename, alti_file, lon0, lat0, h0
):
    """Function for reading mat data from the beast format into a NAV object"""
    dvl_contents = {}
    imu_contents = {}
    ...
    if alti_file == "dvl":
        ...
        try:
            alti_contents["TimestampMeasured"] = (
                pd.to_datetime(
                    alti_contents["log_time"], format="%Y-%m-%dT%H-%M-%S.%fZ"
                ).astype(np.int64)
                // 10**9
            )
        except:
    ...
    try:
        dvl_contents["TimestampMeasured"] = (
            pd.to_datetime(
                dvl_contents["log_time"], format="%Y-%m-%dT%H-%M-%S.%fZ"
            ).astype(np.int64)
            // 10**9
        )
    except:
    ...
    nav = NAV()
    nav.roll = TimeData(
        time=imu_contents["TimestampMeasured"],
    ...
    nav.pos_z = TimeData(
        time=dvl_contents["TimestampMeasured"],
        value=dvl_contents["z"] * 0,
        time_format="unix",
    )

    nav.lat = TimeData(
        time=dvl_contents["TimestampMeasured"], value=lat, time_format="unix"
    )

    nav.lon = TimeData(
        time=dvl_contents["TimestampMeasured"], value=lon, time_format="unix"
    )

    nav.altitude = TimeData(
        time=alti_contents["TimestampMeasured"],
...
def write_nav_data_to_h5(nav, time_offset, config, H5_FILE_PATH):
    """
    takes navigation data and writes it to an H5 file in a synchronized format
    """
    # The time stamp used for writing
    # idk -> maybe yaw has highest frequency?
    ...
    """
    Looking at the write_nav_data_to_h5 function, the purpose is to store navigation data inside the H5 file alongside the hyperspectral data. The comment on line 574 makes this clear:
    # Append the time offset to nav data so that all h5 data is synced to UHI clock
    hsi_synced_nav_timestamp_rov = nav_timestamp_rov + time_offset

    Why the addition happens:
    The H5 file already contains UHI data with UHI timestamps (hyp.hsi_frames_timestamp)

    The navigation data comes in with IMU timestamps (nav_timestamp_rov = nav.yaw.time)

    To store them together consistently, the navigation timestamps need to be converted to the UHI time frame

    So they add the offset to convert: IMU time → UHI time

    The result:
    After this function runs, the H5 file contains:

    Hyperspectral data: already in UHI time
    Navigation data: now also in UHI time (after adding the offset)
    This ensures that when you later read both datasets from the H5 file, their timestamps are in the same time reference frame and can be directly compared/synchronized.

    It's a design choice to use the UHI time frame as the "storage standard" in the H5 files, even though IMU time is used as the "processing standard" elsewhere in the code.
    """

    # This code is preparing to write navigation data into the H5 file. Think of it as organizing data into labeled boxes before storing them.
    # Step 1: Define the "Folder Structure"
    nav_dict_h5_folders = {
        "eul_ZYX": config["HDF.raw_nav"]["eul_ZYX"],
        "position_ecef": config["HDF.raw_nav"]["position"],
        "f": config["HDF.raw_nav"]["timestamp"],
    }

    # Filling with the following data (all using the same time stamps)
    # Step 2: Remove Bad Data
    # This finds any rows where the yaw angle is "NaN" (Not a Number) - basically corrupted data.
    invalid_rows = np.isnan(eul_zyx[:, 2])

    # Step 3: Package the Actual Data
    nav_dict_h5_data = {
    ...
    # wait -> this code dont do shit
    """
    This code is completely pointless because:

    It's just setting keys to their own values!

    nav_dict_h5_folders contains values read from the config
    Then it writes those same values back to the config
    It's like reading a book and then writing down exactly what you just read
    Even if it did something useful, it gets thrown away

    No config.write() call to save to the .ini file
    When the function ends, all changes are lost
    """
    for key, value in nav_dict_h5_folders.items():
        config.set("HDF.raw_nav", key, value=value)
    """
    If position_ecef isn't already in your [HDF.raw_nav] section of the 
    configuration file, this code won't add it. It's just updating existing keys with their own values.
    """

    # This actually writes the navigation data to the H5 file. This is the main purpose and where the real data goes.
    write_data_to_h5_file(
        H5_FILE_PATH, h5_dict_write=nav_dict_h5_folders, h5_dict_data=nav_dict_h5_data
    )
...
def altimeter_data_to_point_cloud(nav, config_uhi, lat0, lon0, h0, true_time_hsi):
    ...
    north, east, down = pm.geodetic2ned(
        lat=lat_alt, lon=lon_alt, h=h_alt, lat0=lat0, lon0=lon0, h0=h0, deg=True
    )

    # Account for the motion
    R_mats_body_ned = RotLib.from_euler(
        "ZYX", np.flip(eul_zyx_alt, axis=1), degrees=True
    ).as_matrix()
...
def point_cloud_to_dem(
    points, config, resolution_dem, lon0, lat0, h0, method="nearest", smooth_DEM=True
):
    ...
    lat_ext, lon_ext, h_ext = pm.ned2geodetic(
        n=extent_ned[:, 0],
        e=extent_ned[:, 1],
        d=extent_ned[:, 1] * 0 + z_values.mean(),
        lat0=lat0,
        lon0=lon0,
        h0=h0,
        deg=True,
    ...
    if crs_dem.is_geographic:
        ...
        transform = Affine(a=res_y, b=0, c=y.min(), d=0, e=-res_x, f=x.max())
        # Setup mesh grid
        grid_y, grid_x = np.meshgrid(
            np.arange(y.min(), y.max(), res_y), np.arange(x.min(), x.max(), res_x)
        )
    elif crs_dem.is_projected:
        # North is then Y
        res_x = resolution_dem * (x.max() - x.min()) / (y_max - y_min)
        res_y = resolution_dem * (y.max() - y.min()) / (x_max - x_min)
        transform = Affine(a=res_x, b=0, c=x.min(), d=0, e=-res_y, f=y.max())
        # Setup mesh grid
        grid_x, grid_y = np.meshgrid(
            np.arange(x.min(), x.max(), res_x), np.arange(y.min(), y.max(), res_y)
        )
    # --------------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    # Transform to wgs 84, aka epsg 4326
    lat_pc, lon_pc, h_pc = pm.ned2geodetic(
        n=points[:, 0],
        e=points[:, 1],
        d=points[:, 2],
        lat0=lat0,
    ...
    points_pc_dem_crs = np.concatenate(
        (x_pc.reshape((-1, 1)), y_pc.reshape((-1, 1)), z_pc.reshape((-1, 1))), axis=1
    )
...
def uhi_beast(config, config_uhi):
    ...
    for h5_index in range(number_of_h5_files):
        ...
        # Build a point cloud
        point_cloud_altimeter = altimeter_data_to_point_cloud(
            nav=nav,
            config_uhi=config_uhi,
        ...
        if h5_index == 0:
            ...
            point_cloud_altimeter_total = np.append(
...
# LIU:
# theres alot of functions used in the uhi_dbe below, but also other functions used probably later on in the pipeline
# chatgpt summary:
"""
uhi_dbe uses 6-7 functions, and they are the ones needed for:

Reading nav + h5 data

Writing nav into h5

Building point cloud from altimeter

Creating a DEM
"""
#
#  the main purpose of the code below is to take the raw data
# from the UHI and then make a DEm using the altitude sensor. Not really that relevant for my work
# heres chatgpt summary:

"""
Prepares navigation and depth data for georeferencing hyperspectral (UHI) images.

What it does:
--------------
1. Reads navigation data (DVL, IMU, Altimeter) for the robot's position, orientation, and altitude.
2. Reads raw hyperspectral image (h5) files that have light measurements.
3. Interpolates the navigation data to match the timestamps of the h5 files.
4. Adds (writes) synchronized position and orientation into each h5 file.
5. Converts the altimeter (altitude) measurements into 3D points in space.
6. Combines all these 3D points into a point cloud.
7. Creates a Digital Elevation Model (DEM) surface from the point cloud by interpolation.

Key inputs:
------------
- config: parsed configuration (.ini) file with paths and options.
- config_uhi: settings with rotation matrices, time offset, etc.

Output:
--------
- Updated h5 files: now contain synchronized position/orientation for each frame.
- A DEM (elevation map) of the seafloor, based on the altimeter data and navigation.

Important notes:
-----------------
- If the robot path is narrow, the DEM will be like a thin strip or line.
- If the robot moves broadly, the DEM will cover a large surface.
- The altimeter gives only 1D depth readings, so creating a full surface depends on motion.
- This step is necessary before orthorectifying the hyperspectral images properly.
"""


def uhi_dbe(config, config_uhi):
    """
    The function prepares UHI (Underwater Hyperspectral Imager) data for georeferencing by
    combining navigation data with hyperspectral images and creating a seafloor elevation model.
    """
    ...
    if alti_file == "alti":
        ...
        search_path_alti = os.path.normpath(
            os.path.join(DVL_DIR, ALTI_PATTERN)
        )  # DVL_DIR = INPUT_DIR ie "Input/" -> so its /Input/alti*
        ALTI_FILE_PATHS = glob.glob(
            search_path_alti
        )  # glob.glob searches for files matching the pattern, returns the path if such files are found eg ["E:/mjosa/Svea2_Day_1/Input/alti_data.csv"]
        ALTI_PATH = ALTI_FILE_PATHS[
            0
        ]  # Assuming there is only one match ie not mutiple "alti_data1.csv", "alti_data2.csv" etc
    elif alti_file == "dvl":
        ALTI_PATTERN = "DVL_*"
        # Locate alti file with nav data
        ...
        print("Alti_file config is not known.")
    # so now we have found the path for the altitude file, which is either an alti file or a dvl file

    # Locate dvl file with nav data -> so bascially Leo wants both alti (which can be dvl) but also a DVL file (which he knows he has)
    search_path_dvl = os.path.normpath(
        os.path.join(DVL_DIR, DVL_PATTERN)
    )  # /Input/DVL_*
    ...
    # ez: takes in sensor input from IMU, DVL, altimeter etc -> spits out NAV object with all the data ie timestamp, position, orientation, altitude -> just like my .csv
    nav = read_nav_from_dvl_imu_alti(
        dvl_filename=DVL_PATH,
        imu_filename=IMU_PATH,
        alti_filename=ALTI_PATH,
        alti_file=alti_file,
        lon0=lon0,
        lat0=lat0,
        h0=alt0,
    ...
    # Search the h5 folder (these are the files to iterate)
    search_path_h5 = os.path.normpath(
        os.path.join(h5_folder, H5_PATTERN)
    )  # H5_PATTERN = "*.h5" -> so its /Input/H5/*.h5
    H5_FILE_PATHS = glob.glob(search_path_h5)

    number_of_h5_files = len(
        H5_FILE_PATHS
    )  # possible since glob.glob returns a list of all the files that match the pattern

    h5_dict_read = {
        # remember that config = configuration.ini file
        "radiance_cube": config["HDF.hyperspectral"][
            "datacube"
        ],  # ie /rawdata/hyperspectral/dataCube
        "hsi_frames_timestamp": config["HDF.hyperspectral"]["timestamp"],
        "fov": config["HDF.calibration"]["fov"],
        "wavelengths": config["HDF.calibration"]["band2wavelength"],
        #'rgb_frames' : config['HDF.rgb']['rgb_frames'],
    ...
    # Liu: they havent implemented photogrammetry using the RGB images from the UHI yet
    # from gref4hsi.utils.photogrammetry_utils import Photogrammetry
    # agisoft_object = Photogrammetry(project_folder = MISSION_PATH, software_type='agisoft')
    # to-do:: Add support for concurrent camera given that images are contained in a h5 folder.
    # Should interface towards ODM as well

    # for each h5 file, read the hyperspectral data and write nav data to it
    for h5_index in range(number_of_h5_files):
        H5_FILE_PATH = H5_FILE_PATHS[h5_index]

        # read the .h5 data and store them into this hyp variaible which is a HyperspectralLite object -> ie acessible via hyp.radiance_cube, hyp.hsi_frames_timestamp etc
        hyp = HyperspectralLite(h5_filename=H5_FILE_PATH, h5_tree_dict=h5_dict_read)
        """
        # Input dictionary:
        h5_dict_read = {
            "radiance_cube": "/rawdata/hyperspectral/dataCube",
            "hsi_frames_timestamp": "/rawdata/hyperspectral/timestamp"
        }

        # After creating HyperspectralLite object:
        hyp = HyperspectralLite("my_file.h5", h5_dict_read)

        # Now you can access:
        hyp.radiance_cube        # Contains the hyperspectral image data
        hyp.hsi_frames_timestamp # Contains the timestamps for each frame

        The HyperspectralLite class is just a convenience wrapper that makes 
        accessing H5 data more intuitive. Instead of having to remember and type 
        out the full H5 hierarchy paths every time, you can just use simple attribute names.
        """

        if h5_index == 0:
            # Camera model is set once, assuming same spatial binning throughout
            binning_spatial = int(
                np.round(SPATIAL_PIXELS / hyp.radiance_cube.shape[1])
            )  # we know the number of spatial pixels in the UHI is 1936, so this is just a way to calculate the binning spatial eg 2

            # Extract from the hyperspectral object
            # Liu: this is just dumb coding, why not just use hyp.fov...
            fov_arr = hyp.fov
    ...
    # saved to path found under "Absolute Paths" -> "dem_path" under the name dummy_dem.tif
    point_cloud_to_dem(
        point_cloud_altimeter_total,
        config,
        resolution_dem=config_uhi.resolution_dem,
...
def uhi_eely(config, config_uhi):
    """
    Modified version of uhi_dbe() for EELY that uses CSV navigation data instead of raw sensor files.
    This function prepares UHI data for georeferencing by combining CSV navigation data
    with hyperspectral images and creating a seafloor elevation model.

    :param config: Configuration object containing file paths and settings
    :param config_uhi: UHI-specific configuration containing CSV navigation data
    """
    MISSION_PATH = config["General"]["mission_dir"]  # Where h5 data is
    ...
    # Get CSV navigation data from config_uhi
    csv_nav_data = config_uhi.csv_nav_data

    print(f"Processing CSV navigation data with {len(csv_nav_data)} records")

    # Convert CSV data to NAV object (this replaces read_nav_from_dvl_imu_alti)
    nav = read_nav_from_csv(
        csv_data=csv_nav_data,
        lon0=lon0,
        lat0=lat0,
        h0=alt0,
    )
    ...
    number_of_h5_files = len(H5_FILE_PATHS)
    print(f"Found {number_of_h5_files} H5 files to process")
    ...
    for h5_index in range(number_of_h5_files):
        ...
        if h5_index == 0:
            ...
            # Creates an XML calibration file that describes the hyperspectral camera's geometry
            set_camera_model(
                config=config,
                config_file_path=config_file_path,
                config_uhi=config_uhi,
        ...
        # Apply time offset to hyperspectral timestamps
        true_time_hsi = hyp.hsi_frames_timestamp - time_offset

        # Write nav data to h5 file
        write_nav_data_to_h5(nav, time_offset, config, H5_FILE_PATH)

        # Build a point cloud from CSV altitude data
        point_cloud_altimeter = altimeter_data_to_point_cloud(
            nav=nav,
            config_uhi=config_uhi,
            true_time_hsi=true_time_hsi,
    ...
    # Use the total point cloud to make a DEM
    if config_uhi.agisoft_process:
        # agisoft_object.load_photos_and_reference()  # Uncomment if using photogrammetry
        pass
    ...
    print("EELY UHI processing completed successfully!")


def read_nav_from_csv(csv_data, lon0, lat0, h0):
    """
    Function for reading CSV navigation data into a NAV object
    Expected CSV columns: timestamp [unix epoch s], latitude [deg], longitude [deg],
                         depth [m], roll [deg], pitch [deg], yaw [deg], altitude [m]
    """
    import pymap3d as pm
    ...
    # Extract timestamps (assuming they're in unix epoch seconds)
    timestamps = csv_data["timestamp [unix epoch s]"].values

    # Attitude data (convert from degrees to radians if needed - check your data)
    nav.roll = TimeData(
        time=timestamps,
        value=np.radians(csv_data["roll [deg]"].values),  # Convert to radians
        time_format="unix",
    )

    nav.pitch = TimeData(
        time=timestamps,
        value=np.radians(csv_data["pitch [deg]"].values),  # Convert to radians
        time_format="unix",
    )

    nav.yaw = TimeData(
        time=timestamps,
        value=np.radians(csv_data["yaw [deg]"].values),  # Convert to radians
        time_format="unix",
    )

    # Position data
    nav.lat = TimeData(
        time=timestamps, value=csv_data["latitude [deg]"].values, time_format="unix"
    )

    nav.lon = TimeData(
        time=timestamps, value=csv_data["longitude [deg]"].values, time_format="unix"
    )

    # Depth (negative Z in NED frame)
    nav.pos_z = TimeData(
        time=timestamps,
        value=-csv_data["depth [m]"].values,  # Negative because NED Z is down
        time_format="unix",
    )

    # Altitude (distance from vehicle to seabed)
    nav.altitude = TimeData(
        time=timestamps,
        value=csv_data["altitude [m]"].values,
        time_format="unix",
    )
