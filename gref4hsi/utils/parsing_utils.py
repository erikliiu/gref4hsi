    ...
    def digital_counts_2_radiance(self, config):
        ...
        if is_calibrated:
            ...
            for i in range(self.dataCube.shape[0]):
                self.dataCubeRadiance[i, :, :] = (
                    self.dataCube[i, :, :] - self.darkFrame
            ...
            Hyperspectral.add_dataset(
                data=self.dataCubeRadiance,
                name=radiance_cube_path,
                h5_filename=self.name,
                overwrite=True,
            )
...
def ardupilot_extract_pose(config, iniPath):
    ...
    Rotation = RotLib.from_euler(
        "ZYX", np.flip(matrix_att[:, 1:4], axis=1), degrees=True
    )
    ...
    linearPositionInterpolator = interp1d(
        time_pos, np.transpose(Position), fill_value="extrapolate"
    )
    ...
    pos_geocsc = np.concatenate(
        (x_ecef.reshape((-1, 1)), y_ecef.reshape((-1, 1)), z_ecef.reshape((-1, 1))),
        axis=1,
    )
    ...
    for i in range(len(time_rot)):
        ...
        data_matrix[i, :] = np.array(
            [time_rot[i], pos[0], pos[1], pos[2], roll, pitch, yaw, rotx, roty, rotz]
        )
    ...
    for filename in sorted(os.listdir(h5_folder)):
        ...
        if filename.endswith("h5") or filename.endswith("hdf"):
            ...
            if rotation_reference_type == "quat":
                quaternion_ref = hyp.get_dataset(
                    h5_filename=path_hdf,
                    dataset_name=config["HDF.raw_nav"]["quaternion"],
                )
                ...
                eul_ref = hyp.get_dataset(
                    h5_filename=path_hdf, dataset_name=config["HDF.raw_nav"]["eul_ZYX"]
                )
                ...
                quaternion_ref = RotLib.from_euler(
                    angles=eul_ZYX_ref, seq="ZYX", degrees=eul_is_degrees
                ).as_quat()

            else:
                raise ValueError(
                    "The rotation reference type must be defined as quat or eul_ZYX"
                )

            is_global_rot = eval(config["HDF.raw_nav"]["is_global_rot"])

            # Parse position
            position_ref = hyp.get_dataset(
                h5_filename=path_hdf, dataset_name=config["HDF.raw_nav"]["position"]
            )

            if position_ref.shape[0] == 3:
                position_ref = np.transpose(position_ref)

            timestamps_imu = hyp.get_dataset(
                h5_filename=path_hdf, dataset_name=config["HDF.raw_nav"]["timestamp"]
            ).reshape(-1)
            ...
            position_interpolated, quaternion_interpolated = interpolate_poses(
                timestamp_from=timestamps_imu,
            ...
            pos_epsg_export = config["Coordinate Reference Systems"][
                "geocsc_epsg_export"
            ]
            ...
            geo_pose_ref = GeoPose(
                timestamps=timestamp_hsi,
            ...
            # # Apply spatial offset from body center to camera center
            # try:
            #     # Read camera offset from config file
            #     translation_x = float(config["Camera"]["translation_body_to_hsi_x"])
            #     translation_y = float(config["Camera"]["translation_body_to_hsi_y"])
            #     translation_z = float(config["Camera"]["translation_body_to_hsi_z"])
            #     translation_body_to_hsi = np.array([translation_x, translation_y, translation_z])

            #     # Apply spatial offset correction for each pose
            #     for i in range(len(position_ref_ecef)):
            #         # Get rotation matrix for this timestamp
            #         rot_matrix_ecef = geo_pose_ref.rot_obj_ecef[i].as_matrix()
            #         # Transform lever arm to ECEF and add to body position
            #         camera_offset_ecef = rot_matrix_ecef @ translation_body_to_hsi
            #         position_ref_ecef[i] += camera_offset_ecef

            # except (KeyError, ValueError):
            #     # If camera offset not defined in config, skip correction
            #     # Poses will represent body center positions at camera timestamps
            #     pass

            # For stacking in CSV
            rot_vec_ecef = geo_pose_ref.rot_obj_ecef.as_euler("ZYX", degrees=True)

            partial_pose = np.concatenate(
                (timestamp_hsi.reshape((-1, 1)), position_ref_ecef, rot_vec_ecef),
                axis=1,
            )
            if is_first:
                # Initialize
            ...
            Hyperspectral.add_dataset(
                data=position_ref_ecef, name=position_ref_name, h5_filename=path_hdf
            )

            # Add camera quaternion
            quaternion_ref_name = config["HDF.processed_nav"]["quaternion_ecef"]
            Hyperspectral.add_dataset(
                data=quaternion_ref_ecef, name=quaternion_ref_name, h5_filename=path_hdf
            )

            # Add time stamps
            time_stamps_name = config["HDF.processed_nav"]["timestamp"]
            Hyperspectral.add_dataset(
                data=timestamp_hsi, name=time_stamps_name, h5_filename=path_hdf
            )
...
def export_pose(config_file):
    ...
    if pose_export_type == "h5_embedded":
        reformat_h5_embedded_data_h5(config=config, config_file=config_file)
...
def export_model(config_file):
    ...
    if model_export_type == "agisoft":
        ...
        pass
    # here the dem is the dem file that was created earlier with the altitude data
        ...
        if os.path.exists(file_path_3d_model):
            ...
            dem_2_mesh(
                path_dem=file_path_dem, model_path=file_path_3d_model, config=config
            )
    elif model_export_type == "geoid":  # see comment below
        file_path_dem = config["Absolute Paths"]["dem_path"]
        ...
        crop_geoid_to_pose(
            path_dem=file_path_dem, config=config, geoid_path=file_path_geoid
        )

        # Make into a 3D model
        dem_2_mesh(path_dem=file_path_dem, model_path=file_path_3d_model, config=config)
    """   
    dont really undestand this one, even chatgpt said i dont need to care abou tthe geoid for my purpose, the dem_file is what we are after 
    """
