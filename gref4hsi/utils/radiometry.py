...
def compute_backscatter(
    B_p, b, b_w, b_p, d_pix, pos_seabed, p_si, dir_s0, sigma_l, I_0, I_hat, k, pixel_nr
):
    ...
    l_si_b = np.linalg.norm(r_si_b, axis=2).reshape(
        (r_si_b.shape[0], r_si_b.shape[1], 1)
    )  # t by m by 1 or something

    r_b_h = -z * d_pix  # t by m by 3

    l_b_h = np.linalg.norm(r_b_h, axis=2).reshape(
        (r_si_b.shape[0], r_si_b.shape[1], 1)
    )  # t by m by 1 or something

    BP_norm, BP = beam_pattern(
        dir_s0=dir_s0, dir=r_si_b.reshape((-1, 3)), sigma=sigma_l, I_0=I_0, I_hat=I_hat
    ).reshape((r_si_b.shape[0], r_si_b.shape[1], I_hat.shape[0]))

    E_i = (
        BP * (1 / (l_si_b**2)) * np.exp(-k * l_si_b)
    )  # Values of incident irradiance for all directions and all perpendicular
    ...
    beta_p_interp = fournier_forand(
        B_p
    )  # Assumed constant backscatter fraction across wl. Is a log-log interpolator

    beta_p = np.exp(beta_p_interp(np.log(psi))).reshape(
        (r_si_b.shape[0], r_si_b.shape[1], 1)
    )  # Due to the interpolation in log-log domain. t by m

    beta_w = compute_beta_w(psi).reshape(
        (r_si_b.shape[0], r_si_b.shape[1], 1)
    )  # t by m
    ...
    integral = (
        np.cumsum(integrand, axis=0) * z_res
    )  # Summing along the z axis weighted by z_res. Should return a t by m by k.
...
def fournier_forand(B_p):
    ...
    delta = (4 * (np.sin(psi / 2)) ** 2) / (
        3 * (n - 1) ** 2
    )  # Correct, Mobley 2002 eq 2
    ...
    beta_ff = (
        beta_ff_t1 * (beta_ff_t2 + beta_ff_t3 * beta_ff_t4) + beta_ff_t5 * beta_ff_t6
    )  ## Formula t1*(t2+t3*t4) + t5*t6

    # Instantiate an interpolation object that describes the function
    beta_ff_interp = interp1d(
        x=logPsi, y=np.log(beta_ff), kind="cubic", fill_value="extrapolate"
    )
...
class Radiometry:
    def __init__(self, config, data1, data2):
        ...
        df_light = pd.read_csv(
            "C:/Users/haavasl/PycharmProjects/newGit/TautraReflectanceTools/Misc/DataMultiSeaLite.txt",
            header=None,
        )
        ...
        df_iop = pd.read_csv(
            path_iop_water, header=9, sep=","
        )  # Should be interpolated

        df_coral = pd.read_csv(
            "C:/Users\haavasl\PycharmProjects/newGit\TautraReflectanceTools\Misc\Coral_spectra_lab.csv",
            sep=";",
        )

        df_refl_plaque = pd.read_csv(
            "C:/Users\haavasl\PycharmProjects/newGit\TautraReflectanceTools\Misc/reflectance_plaque.csv"
        )

        interpol_coral = interp1d(
            df_coral.iloc[:, 0].astype(np.float64),
            np.transpose(df_coral.iloc[:, 2:4].astype(np.float64)),
            fill_value="extrapolate",
        )
        interpol_iop = interp1d(
            df_iop.iloc[:, 0].astype(np.float64),
            np.transpose(df_iop.iloc[:, 1:3].astype(np.float64)),
        )
        interpol_light = interp1d(
            df_light.iloc[:, 0],
            np.transpose(df_light.iloc[:, 1]),
            fill_value="extrapolate",
        )

        interpol_refl = interp1d(
            df_refl_plaque.iloc[:, 0], np.transpose(df_refl_plaque.iloc[:, 1])
        )
        ...
        self.Var = Var(
            ...
            normals1=data1.normals,
            pixel_nr1=data1.pixel_nr,
            bands1=data1.bands,
            angles1=data1.angles,
            RF1=data1.RF,
            t_exp1=data1.t_exp,
            points2=data2.points,
            normals2=data2.normals,
            pixel_nr2=data2.pixel_nr,
            bands2=data2.bands,
        ...
        self.Const = Const(
            ...
            n_g=n_g,
            n_w=n_w,
            dir_s1=np.array([0, 0, 1]),
            dir_s2=np.array([0, 0, 1]),
            pos_s1=np.array([0.35, 0, 0]),
            pos_s2=np.array([-0.35, 0, 0]),
            I_hat=I_hat,
            refl_gt=refl_gt,
            refl_gt_coral=refl_gt_coral,
        )

        Param = namedtuple(
            "Param", ["G", "X", "Y", "B_p", "alpha", "I_0", "sigma_l", "k_l"]
        )
        self.Param = Param(
            G=0.1, X=0.1, Y=1, B_p=0.01, alpha=0, I_0=1, sigma_l=1, k_l=0
        )
    ...
    def set_parameters(self, G, X, Y, B_p, alpha, I_0, sigma_l, k_l):
        ...
        self.Param = Param(
            G=G, X=X, Y=Y, B_p=B_p, alpha=alpha, I_0=I_0, sigma_l=sigma_l, k_l=k_l
        )
    ...
    def two_way_model_simple(self, id, is_skogn):
        ...
        BP1_norm, BP1 = beam_pattern(
            dir_s0=dir_s10, dir=dir_s1, sigma=sigma_l, I_0=I_0, I_hat=I_hat
        )
        BP2_norm, BP2 = beam_pattern(
            dir_s0=dir_s20, dir=dir_s2, sigma=sigma_l, I_0=I_0, I_hat=I_hat
        )
        ...
        if include_backscatter:
            ...
            if is_skogn:
                L_s1 = compute_backscatter(
                    B_p=B_p,
                    b=b.reshape((1, 1, k_bands)),
                    b_w=b_w.reshape((1, 1, k_bands)),
                    b_p=b_p.reshape((1, 1, k_bands)),
                    d_pix=d_pix,
                    pos_seabed=pos_seabed,
                    p_si=pos_s1,
                    dir_s0=dir_s10,
                    sigma_l=sigma_l,
                    I_0=I_0,
                    I_hat=I_hat,
                    k=k.reshape((1, 1, k_bands)),
                    pixel_nr=pixel_nr,
                )
