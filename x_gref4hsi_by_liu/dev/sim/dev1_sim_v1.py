"""
Multi-H5 UHI segments with slider, showing IMU & Camera frames and body-axis arrows.
...
Run:
    python visualize_multi_h5_uhi_imu_cam.py
    python visualize_multi_h5_uhi_imu_cam.py --frames 400
"""
...
# ========= User-tunable orientation settings =========
# Yaw convention:
#   "heading_from_north_cw" means yaw is a compass heading (deg), 0=N, 90=E, clockwise positive.
#   "enu_yaw_from_east_ccw" means yaw is radians from East CCW (standard math/ROS ENU).
ORI_CONVENTION = "heading_from_north_cw"  # or "enu_yaw_from_east_ccw"
RPY_UNITS = "deg"  # "deg" or "rad"
CAMERA_FWD_OFFSET_M = 2.5  # camera is 2.5 m ahead of IMU along body +X
AXIS_ARROW_LEN_M = 1.5  # length of body-axis arrows for visualization
# =====================================================
...
def _extract_rpy(interp_nav):
    ...
    raise KeyError(
        "Could not find roll/pitch/yaw in interpolated nav. "
        "Expected columns like ['roll','pitch','yaw'] or ['roll_deg','pitch_deg','yaw_deg']."
    )
...
def _body_to_enu_rotation(roll_rad, pitch_rad, yaw_in):
    ...
    Build R_ENU_from_BODY for a single sample given:
      ...
      - yaw_in depending on ORI_CONVENTION:
          * heading_from_north_cw: yaw_in is heading (deg/rad per RPY_UNITS)
          * enu_yaw_from_east_ccw: yaw_in is yaw about +Z ENU from +X (East), CCW, radians/deg per RPY_UNITS
    ...
    if ORI_CONVENTION == "heading_from_north_cw":
        # Convert heading (N=0, E=90, clockwise) to ENU yaw (E=0, CCW)
        if RPY_UNITS == "deg":
            yaw_val = np.deg2rad(90.0 - yaw_in)
        else:
            yaw_val = (np.pi / 2.0) - yaw_in
    else:
        # already ENU yaw from East CCW
    ...
    # Rx * Ry * Rz? We want body->ENU with ZYX intrinsic equals Rz(yaw)*Ry(pitch)*Rx(roll) extrinsic
...
def _load_segments(num_frames=None):
    ...
    Returns:
      ...
      segments: list of dicts, each with keys:
        name, color, x_imu, y_imu, z_imu,  rpy arrays, lookup_ts_used,  bounds strings
...
def plot_multi_segments_with_slider_and_frames(
    full_xyz, segments, combined, title_extra=""
):
    """
    3D plot:
      ...
      - Moving IMU point (black) and Camera point (magenta)
    ...
    ttl = f"Mission with UHI segments highlighted   {title_extra}\nGlobal lookup range: {global_start} to {global_end}"
    ...
    def _update_frame(i):
        ...
        # Build rotation body->ENU for this pose
        roll_rad = _to_radians(roll_arr[i])
        pitch_rad = _to_radians(pitch_arr[i])
        yaw_in = yaw_arr[i]
        R = _body_to_enu_rotation(roll_rad, pitch_rad, yaw_in)

        # Camera position: IMU + R * [2.5, 0, 0]
        cam_offset_body = np.array([CAMERA_FWD_OFFSET_M, 0.0, 0.0])
        cam_offset_enu = R @ cam_offset_body
        cxm, cym, czm = (
            px + cam_offset_enu[0],
            py + cam_offset_enu[1],
            pz + cam_offset_enu[2],
        ...
        x_tip = np.array([px, py, pz]) + (R @ np.array([ax_len, 0.0, 0.0]))
        y_tip = np.array([px, py, pz]) + (R @ np.array([0.0, ax_len, 0.0]))
        z_tip = np.array([px, py, pz]) + (R @ np.array([0.0, 0.0, ax_len]))

        # Update artists
        imu_pt.set_data([px], [py])
        imu_pt.set_3d_properties([pz])
        cam_pt.set_data([cxm], [cym])
        cam_pt.set_3d_properties([czm])
        ...
        # Color IMU point by current segment for clarity
    ...
    # Play/Pause
    ax_button = fig.add_axes([0.82, 0.05, 0.1, 0.04])
...
def main():
    parser = argparse.ArgumentParser(
        description="Multi-H5 UHI with IMU/Cam frames and body axes"
    )
    parser.add_argument(
    ...
    title_extra = f"(offset {float(config.TIME_OFFSET_SEC):+.1f}s)"
    plot_multi_segments_with_slider_and_frames(
        full_xyz, segments, combined, title_extra=title_extra
    )
