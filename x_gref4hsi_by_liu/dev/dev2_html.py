        ...
        for dset_path in _DATASET_CANDIDATES:
            try:
                ...
                if d.ndim == 3 and d.shape[-1] == 3:
                    ...
                    # For visualization, we can treat as single track
    ...
    try:
        with h5py.File(h5_path, "r") as f:
            ...
            if dset_path not in f:
                dset_path = "processed/radiance/dataCube"  # Fallback
            if dset_path not in f:
...
def add_hsi_rgb_overlay(
    ...
    Similar to plot_georef_rgb but for interactive HTML maps.

    ⚠️ PERFORMANCE WARNING:
    - stride_tracks=2, stride_slits=2 → ~800k points → 100+ MB HTML file (may crash browser!)
    - stride_tracks=5, stride_slits=5 → ~128k points → ~20 MB (acceptable)
    - stride_tracks=10, stride_slits=10 → ~32k points → ~5 MB (recommended)
    - stride_tracks=20, stride_slits=20 → ~8k points → ~1 MB (fast)
    """
    files = []
    if hsi_h5_dir:
    ...
    for idx, fp in enumerate(files, 1):
        ...
        # Add colored points with stride
        T, S = lat.shape

        # Calculate total points to add for progress tracking
        ...
        n_points = 0
        progress_update = max(1, total_cells // 20)  # Update every 5%

        print(f"adding {total_cells:,} cells...", end="", flush=True)
        ...
        for idx_i, i in enumerate(track_indices):
            for idx_j, j in enumerate(slit_indices):
                ...
                folium.CircleMarker(
                    ...
                    weight=0,
                    tooltip=f"{basename} | track {i}, slit {j}<br>RGB: ({r:.3f}, {g:.3f}, {b:.3f})",
                ).add_to(grp)
                n_points += 1
                ...
                if cell_num > 0 and cell_num % progress_update == 0:
                    ...
                    print(
                        f"\r  [{idx}/{len(files)}] {basename}... {pct}% ({n_points:,} points)",
                        end="",
                        flush=True,
                    )

        print(
            f"\r  [{idx}/{len(files)}] {basename}... ✅ {n_points:,} colored points     "
        )
        ok_files += 1
...
def add_hsi_from_ecef(
    ...
    Handles both gridded (T,S,3) and flattened (N,3) ECEF point cloud formats.
    """
    files = []
    if hsi_h5_dir:
        files = _discover_h5_files(hsi_h5_dir, recursive=recursive)
...
def build_map_with_mbes(
    ...
    hsi_add_rgb=True,  # NEW: Add RGB colored overlay
    hsi_rgb_layer_name="HSI RGB",
    hsi_rgb_stride_tracks=2,
    hsi_rgb_stride_slits=2,
    ...
    # 8b) HSI RGB overlay (colored spectral data)
    ...
    file_size_mb = os_module.path.getsize(output_html) / (1024 * 1024)
...
if __name__ == "__main__":
    # All paths now from config.py
    print("=" * 70)
    print("Building interactive map with MBES + HSI RGB overlay")
    print("=" * 70)
    print("⚡ Performance: stride=10 → ~32k RGB points (~5 MB HTML)")
    print("   (Adjust hsi_rgb_stride_tracks/slits below to change density)")
    print("=" * 70)
    ...
    build_map_with_mbes(
        ...
        hsi_stride_tracks=10,  # Larger stride for outline
        hsi_stride_slits=10,
        hsi_add_lines=True,
        hsi_add_points=False,
        hsi_point_cluster=False,
        hsi_color="#ffff00",  # Yellow outline
        # HSI RGB overlay (actual spectral data colors)
        hsi_add_rgb=True,
        hsi_rgb_layer_name="HSI RGB (spectral)",
        hsi_rgb_stride_tracks=10,  # Reduced from 2 to 10 for performance (98% fewer points!)
        hsi_rgb_stride_slits=10,  # Reduced from 2 to 10 for performance
        hsi_rgb_red_wl=654.2,
        hsi_rgb_green_wl=560,
        hsi_rgb_blue_wl=440.3,
        hsi_rgb_normalize=True,
    ...
    print("   - 'HSI RGB (spectral)' shows the actual hyperspectral colors")
    print("   - 'HSI footprint (outline)' shows the coverage area")
    print()
    print("⚙️  Performance settings:")
    print(f"   RGB stride: tracks=10, slits=10 (~32k points)")
    print(f"   To reduce further: Edit stride values in dev2_html.py")
    print(f"   - stride=20 → ~8k points (4x faster)")
    print(f"   - stride=5 → ~128k points (4x slower, may crash browser)")
    print("=" * 70)
    print()
