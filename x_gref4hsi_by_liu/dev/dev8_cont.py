...
Four figures shown together. MBES is transformed from its source CRS to NED.
"""
...
# ----------------------------
# Minimal MBES Detrending Class (with CRS fix)
# ----------------------------
...
class MBESDetrender:
    ...
    def load(self):
        with rasterio.open(self.tif_path) as src:
            ...
            tf = src.transform
            bounds = src.bounds
            self.crs = src.crs  # <-- record the CRS
...
# ----------------------------
# UHI-on-MBES overlay (CRS-safe; both in NED)
# ----------------------------
def plot_uhi_mask_on_mbes(
    ...
    """
    Plot UHI validity mask over detrended MBES residuals, both in NED.

    mbes_detrender : MBESDetrender (must have .residuals and .crs)
    uhi_x_ned, uhi_y_ned : UHI NED East/North (T,S) arrays
    uhi_mask : boolean/0-1 mask same shape
    origin_lat/lon/h : NED origin
    """
    ...
    if getattr(mb, "crs", None) is None:
        raise ValueError(
            "MBESDetrender.crs is None. Did you call load() on MBESDetrender?"
        )
    ...
    print(
        f"   UHI X (NED East) range: [{np.nanmin(x_uhi_ned):.2f}, {np.nanmax(x_uhi_ned):.2f}] m"
    )
    print(
        f"   UHI Y (NED North) range: [{np.nanmin(y_uhi_ned):.2f}, {np.nanmax(y_uhi_ned):.2f}] m"
    )

    # Convert MBES grid from its source CRS -> WGS84 -> ECEF -> NED
    print("   🔄 Converting MBES grid from source CRS to NED...")
    ...
    print(
        f"   MBES X (NED East) range: [{np.nanmin(mbes_e_ned):.2f}, {np.nanmax(mbes_e_ned):.2f}] m"
    )
    print(
        f"   MBES Y (NED North) range: [{np.nanmin(mbes_n_ned):.2f}, {np.nanmax(mbes_n_ned):.2f}] m"
    )
